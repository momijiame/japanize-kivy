...
Publishing a release to [PyPI](https://pypi.org/project/japanize-kivy/) means creating a
...
is no version to edit in `pyproject.toml`, and `japanize_kivy.__version__` reads back the
metadata the build produced rather than being the thing that feeds it.

| state of the checkout | reported version |
| --- | --- |
| exactly on `v0.2.0`, clean tree | `0.2.0` |
| a few commits past it | `0.2.1.dev3+g1a2b3c4` |
| on a tagged commit with a dirty tree | `0.2.1.dev0+g1a2b3c4.d20260918` |

The `v` prefix is stripped by setuptools-scm's default tag pattern, so keep naming tags
`vX.Y.Z`. The one tag that predates this workflow, `0.1.1`, has no prefix; leave it as it
is and prefix every new one.

## Steps

### 1. Get the release content onto `master`

Whatever `master` points at when the tag is created is what gets built, so merge
...
Check that the `test` workflow is green for that commit.

### 2. Create the tag

The existing tag is annotated, so keep it that way:
...
```console
$ gh run watch
$ pip index versions japanize-kivy
```

The workflow builds an sdist and a wheel with `uv build` on Python 3.13 and uploads them
with `pypa/gh-action-pypi-publish`, which also attaches PEP 740 attestations — provenance
PyPI can verify, and something trusted publishing is a precondition for. The upload is
recorded under the repository's Deployments tab.

## Things to know

**A version number cannot be reused.** PyPI rejects a re-upload of a version it already
has. Tagging the wrong commit and publishing means `0.2.0` is spent for good, and the
fix is to move on to `0.2.1` — deleting the release and the tag does not free the
number. Hence step 3.
...
**The font makes the wheel large.** The bundled IPAex Gothic is about 6 MB, so the wheel
is around 4 MB. That is expected; it is not a packaging mistake.
